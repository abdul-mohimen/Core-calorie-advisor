...
(async () => {
  console.log('Starting member portal visual audit...');
  
  const browser = await chromium.launch({ headless: true });
  const context = await browser.newContext();
  const page = await context.newPage();
  
  // Login via backdoor script to avoid UI flakiness
  console.log('Logging in as member via session injection...');
  await page.goto('http://127.0.0.1/Core%20calorie%20advisor/_audit/set_session.php');
  await page.waitForLoadState('networkidle');
  console.log('Session injected.');

  const outDir = path.join(__dirname, 'compare', 'member');
  if (!fs.existsSync(outDir)) {
    fs.mkdirSync(outDir, { recursive: true });
  }

  // Go explicitly to member portal just in case
  await page.goto('http://127.0.0.1/Core%20calorie%20advisor/portals/member.php');
  ...
  await browser.close();
  console.log('Member portal audit completed.');
})();
