...
(async () => {
  console.log('Starting patient portal visual audit...');
  ...
  console.log('Logging in as patient via session injection...');
  await page.goto('http://127.0.0.1/Core%20calorie%20advisor/_audit/set_session.php');
  await page.waitForLoadState('networkidle');
  console.log('Session injected.');

  const outDir = path.join(__dirname, 'compare', 'patient');
  if (!fs.existsSync(outDir)) {
    fs.mkdirSync(outDir, { recursive: true });
  }

  // Go explicitly to patient portal
  await page.goto('http://127.0.0.1/Core%20calorie%20advisor/portals/patient.php');
  ...
  await browser.close();
  console.log('Patient portal audit completed.');
})();
