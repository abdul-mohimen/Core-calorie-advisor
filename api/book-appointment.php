...
if (!$st->fetch())            { flash('err', 'Selected trainer or doctor was not found.'); redirect('pages/trainers.php'); }
if ($goal === '' || !$date)   { flash('err', 'Both consultation goal and appointment date are required.'); redirect('pages/trainers.php'); }

$ts = strtotime($date);
if (!$ts)                     { flash('err', 'Invalid appointment date format.'); redirect('pages/trainers.php'); }
...
flash('ok', '✔ Appointment request submitted! You will be notified once reviewed.');
